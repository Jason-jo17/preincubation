...
export function MindMap5W1H({ tool, progress, onDataSaved, isReadOnly }: MindMap5W1HProps) {
  const [nodes, setNodes, onNodesChange] = useNodesState(progress?.submittedData?.nodes || progress?.data?.nodes || initialNodes)
  const [edges, setEdges, onEdgesChange] = useEdgesState(progress?.submittedData?.edges || progress?.data?.edges || initialEdges)
                      ...
                      {getChildren(cat.id).map(child => (
                        <div key={child.id} className="flex gap-2 items-start group">
                          ...
                          <Textarea
                            ...
                            readOnly={isReadOnly}
                            className={`min-h-[2.5rem] py-1 px-2 text-sm resize-none bg-transparent hover:bg-white focus:bg-white border-transparent hover:border-input focus:border-input transition-colors ${isReadOnly ? 'cursor-default' : ''}`}
                            rows={1}
                          />
                          {!isReadOnly && (
                            <Button
