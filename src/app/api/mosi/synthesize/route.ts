    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      if (recordingUrl) {
        ...
        try {
          ...
          console.log('Primary fetch failed, attempting fallback...', primaryErr.message);
          // Fallback logic for local development if needed
...
`;
    ...
    // MSME BRIDGE TRIGGER
    const supabaseUrl = process.env.NEXT_PUBLIC_MOSI_SUPABASE_URL;
    const supabaseAnonKey = process.env.NEXT_PUBLIC_MOSI_SUPABASE_ANON_KEY;

    if (supabaseUrl && supabaseAnonKey && synthesisResult.opportunities && synthesisResult.opportunities.length > 0) {
      ...
      try {
        const session_data = {
          ...
          company_id: (stakeholder as StakeholderInput).company_id || 'ceed-1',
          status: 'Published',
