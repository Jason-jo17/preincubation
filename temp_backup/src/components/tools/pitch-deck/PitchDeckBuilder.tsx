...
export default function PitchDeckBuilder({ tool, progress }: PitchDeckBuilderProps) {
    const [state, setState] = useState<PitchDeckState>(() => {
        // 1. Try DB-saved progress
        if (progress?.submittedData?.decks) {
            return {
                view: 'dashboard',
                decks: progress.submittedData.decks,
                currentDeckId: progress.submittedData.currentDeckId || null,
                activeSlideId: null,
                version: progress.submittedData.version || 1
            }
        }
        // 2. Try legacy progress.data
        if (progress?.data) {
            try {
                const data = typeof progress.data === 'string' ? JSON.parse(progress.data) : progress.data
                if (data.decks) return data
            } catch (e) {}
        }
        // 3. Fallback to localStorage (migration path)
        try {
            const saved = localStorage.getItem(`pitch_deck_${tool.id}`)
    ...
    useEffect(() => {
        const saveTimeout = setTimeout(async () => {
            // Save to DB via sprint submission system
            await saveToolData(tool.toolId, { 
                decks: state.decks, 
                currentDeckId: state.currentDeckId,
                version: state.version
            })
            // Keep localStorage as offline fallback
            try { localStorage.setItem(`pitch_deck_${tool.id}`, JSON.stringify(state)) } catch {}
        }, 1500)
        return () => clearTimeout(saveTimeout)
    }, [state, tool.id, tool.toolId])
