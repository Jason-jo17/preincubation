...
export function PersonaJourneyTool({ tool, progress, onDataSaved, isNewIteration, submissionId }: Props) {
  const initialData = progress?.submittedData || progress?.data || INITIAL_DATA
  const [data, setData] = useState<PersonaJourneyData>(initialData)
      ...
      <div className="h-16 shrink-0 border-b bg-white flex items-center justify-between px-6 z-10">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col gap-1 pr-4">
             <input
                type="text"
                placeholder="Entry Name (e.g. Persona X)"
                className="text-xs font-bold bg-transparent border-none focus:ring-0 p-0 w-32 truncate"
                value={iterationName}
