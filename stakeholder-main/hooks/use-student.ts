import { useQuery } from "@tanstack/react-query"
import { StakeholderProfile } from "@/types"

export function useStudentProfile() {
    return useQuery({
        queryKey: ['student-profile'],
        queryFn: async () => {
            const res = await fetch('/api/student/journey')
            if (!res.ok) throw new Error("Failed to fetch student profile")
            const journey = await res.json()
            return {
                id: journey.id,
                trlLevel: journey.trlLevel,
                sprints: journey.sprints || [],
                metrics: journey.metrics || {},
                user: journey.user,
                _count: {
                    interactions: journey.user?.interactionsAsInitiator?.length || 0
                },
                interactions: journey.user?.interactionsAsInitiator?.map((i: any) => ({
                    id: i.id,
                    type: i.type,
                    subject: i.notes || "Recorded Activity",
                    occurredAt: i.occurredAt
                })) || [],
                manager: journey.user?.studentProfile?.manager
            }
        }
    })
}

export function useMyStakeholders() {
    return useQuery({
        queryKey: ['my-stakeholders'],
        queryFn: async () => {
            // Mock data
            return [
                {
                    id: "sh-1",
                    user: { name: "Rajesh Kumar", avatar: null },
                    district: "Mangaluru",
                    designation: "Director",
                    organization: "Tech Solutions",
                    sectors: [{ id: "1", name: "Technology", color: "#8B5CF6" }],
                    problemStatements: [],
                    _count: { interactions: 3, linkedStakeholders: 2 },
                    verificationStatus: 'verified'
                } as any as StakeholderProfile[]
            ]
        }
    })
}

export function useMyValuePropositions() {
    return useQuery({
        queryKey: ['my-value-props'],
        queryFn: async () => {
            return [
                {
                    id: "vp-1",
                    customerJobs: ["Improve crop yield"],
                    pains: ["Pest attacks", "Water shortage"],
                    gains: ["Higher income", "Sustainability"],
                    painRelievers: ["Smart monitoring"],
                    gainCreators: ["Data driven insights"],
                    productsServices: ["AgriSense IoT"],
                    validationStatus: "validated",
                    createdAt: new Date().toISOString()
                }
            ]
        }
    })
}
