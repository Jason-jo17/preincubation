...
export function StageCard({ 
    ...
    isUnlocked = false,
    nudgeMap = {}
}: StageCardProps & { toolSubmissionsMap?: any }) {
    ...
    const completedTools = tools.filter((tool: any) => {
        ...
        return tp?.status === 'completed' || (toolSubmissionsMap?.[tool.toolId]?.some((s: any) => s.status === 'gate_passed'))
    }).length
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {tools.map((tool: any) => (
                    <ToolCard
                        ...
                        submissions={toolSubmissionsMap?.[tool.toolId] || []}
                        onStart={onStartTool}
                        onOpen={onOpenTool}
                        isStageLocked={isLocked}
                        isAdmin={isAdmin}
                        isUnlocked={isUnlocked}
                        nudgeStatus={nudgeMap?.[tool.toolId]}
                    />
                ))}
            </div>
        </div>
