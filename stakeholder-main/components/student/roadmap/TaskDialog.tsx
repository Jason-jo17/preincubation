...
import { CheckCircle2, Circle, AlertCircle, ExternalLink, Minimize, Youtube, FileText, Globe } from "lucide-react"
import { submitTask } from "@/app/actions/roadmap"
import { toast } from "sonner"
import { cn } from "@/lib/utils"

import { NudgeBanner } from "@/components/student/NudgeBanner"
            ...
            >
                ...
                <Tabs defaultValue="tool" className={cn(
                    ...
                    <div className={cn(
                        ...
                        <TabsContent value="resources" className={isMaximized ? "h-[calc(100vh-180px)]" : "h-[400px] mt-4"}>
                            <div className="space-y-4 p-4">
                                {progress?.resources && progress.resources.length > 0 ? (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {progress.resources.map((res: any, idx: number) => (
                                            <a key={idx} href={res.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-4 rounded-xl border bg-card hover:border-primary/50 transition-colors group tracking-tight shadow-sm">
                                                <div className="h-10 w-10 shrink-0 rounded-full bg-primary/10 flex items-center justify-center text-primary group-hover:scale-110 transition-transform">
                                                    {res.type === 'video' ? <Youtube className="h-5 w-5" /> : 
                                                     res.type === 'document' ? <FileText className="h-5 w-5" /> : 
                                                     res.type === 'webview' ? <Globe className="h-5 w-5" /> :
                                                     <ExternalLink className="h-5 w-5" />}
                                                </div>
                                                <div className="flex-1 overflow-hidden">
                                                    <h4 className="font-bold text-sm group-hover:text-primary transition-colors truncate">{res.title || "Resource Link"}</h4>
                                                    <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-black opacity-70 mt-1">{res.type}</p>
                                                </div>
                                    ...
                                    <div className="text-center py-12 opacity-50 border-2 border-dashed rounded-xl">
                                        <p className="font-black uppercase tracking-widest text-sm text-foreground/50">No required learning resources</p>
                                    </div>
                                )}
                            </div>
                        </TabsContent>
                        <TabsContent value="tool" className="h-full w-full m-0 p-0 border-none">
                            {/* Nudge banner renders ABOVE the tool canvas — always visible at top */}
                            {nudge && nudge.nudgeType !== 'CLEAR' && (
                                <div className={isMaximized ? "px-4 pt-4" : "px-1 pt-2 pb-3"}>
                                ...
                                <div className="md:col-span-2 space-y-6">
                                    ...
                                    <div className="space-y-4">
                                        {tool.tasks?.length > 0 ? (
                                            ...
                                            <div className="text-sm text-muted-foreground p-4 bg-slate-50 border rounded-lg">
                                                No specific review requirements checklisted for this tool yet. Focus on the main interactive tool above.
                                            </div>
                                        )}
                                    </div>
                                </div>
