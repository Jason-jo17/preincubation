...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.log("[CRL ASSESSMENT] Starting for evidenceId:", evidenceId);
    ...
    if (!evidence) {
      console.error("[CRL ASSESSMENT] Evidence not found:", evidenceId);
      return NextResponse.json({ error: "Evidence not found" }, { status: 404 })
    }
    
    if (evidence.journey.userId !== session.user.id) {
      console.error("[CRL ASSESSMENT] Unauthorized access by user:", session.user.id);
      return NextResponse.json({ error: "Unauthorized" }, { status: 403 })
    }

    // Auto-rejection checks
    if (evidence.evidenceText.length < 50) {
      console.log("[CRL ASSESSMENT] Evidence too short, auto-rejecting.");
      const updated = await prisma.cRLEvidence.update({
        where: { id: evidenceId },
        data: {
          status: 'rejected',
    ...
    const systemPrompt = `You are a Commercial Readiness Level (CRL) expert.
    ...
    console.log("[CRL ASSESSMENT] Calling AI with prompt length:", (systemPrompt + prompt).length);
    ...
    if (!response.text) {
      console.error("[CRL ASSESSMENT] Empty response from AI");
      throw new Error("No response from AI")
    }

    console.log("[CRL ASSESSMENT] AI Response received:", response.text);
    ...
    console.log("[CRL ASSESSMENT] Updating database with status:", result.isInsufficientData ? 'insufficient' : (passed ? 'approved' : 'rejected'));
    ...
    console.log("[CRL ASSESSMENT] Completed successfully for evidenceId:", evidenceId);
    return NextResponse.json({ success: true, evidence: updated })

  } catch (error) {
    console.error("[CRL ASSESSMENT] ERROR:", error)
